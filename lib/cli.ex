...
defmodule Ragnarok.CLI do
    require Logger

    def main(args) do
        Application.get_env(:ragnarok, :master_node)
        |> Node.start()

        Application.get_env(:ragnarok, :slave_nodes)
    ...
    defp do_requests(n_requests, url, nodes) do
        ...
        |> Enum.flat_map(fn node -> 
            1..req_per_node |> Enum.map(fn _ -> 
                Task.Supervisor.async({Ragnarok.TasksSupervisor, node}, Ragnarok.Worker, :start, [url])
            end)
        end)
